...
<p align="center">
  ...
  <a href="docs/CI.md"><img alt="coverage: 82.53%" src="https://img.shields.io/badge/coverage-82.53%25-success"></a>
  ...
  <a href="scripts/mutation-gate.sh"><img alt="mutations killed: 186/186" src="https://img.shields.io/badge/mutations%20killed-186%2F186-success"></a>
...
| Checked | 343 unit tests and 28 doctests, 82.53% line coverage against an 80% floor, **186 mutations each re-broken and each required to turn its named test red**, and **57 violations planted in a scratch repository that the gates must catch citing the right rule**. None of that is evidence about hardware; all of it is evidence about the code |
...
data exists on one device. An unreachable replica is never filtered out as
noise; **`NeverReplicated` is a distinct state from a large lag**, because
...
A full run exercises **343 unit tests and 28 doctests** (2 ignored — the two
snapshot regenerators), kills **186 mutations**, catches **57 planted violations**,
and measures **82.53% line coverage** against a floor of 80. That is a suite that
...
| **Coverage** | Production line coverage below **80%** (measured: 82.53%), with test files excluded so the figure is not inflated by the coverage of the tests themselves. A missing coverage tool is a failure, never a pass |
