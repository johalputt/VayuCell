...
| **Actions** | A workflow reference that is **not pinned to a full commit SHA**, or one naming a commit that cannot be fetched. A tag is whatever its owner repoints it at tomorrow, and repointing produces no diff here. Also an extraction pattern that has gone stale and found nothing. Without network it prints `UNVERIFIED`; CI requires the network so the authoritative run cannot skip it |
