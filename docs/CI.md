...
4. **Four supply-chain scorecard findings are not fixable by a commit**, and are
   ...
   | **Code-Review** | Was `0/29 approved changesets`, because changes were pushed straight to `main`. As of this change every change goes through a pull request — see `CONTRIBUTING.md`. The score moves as reviewed changesets accumulate; it is not something a commit can assert |
